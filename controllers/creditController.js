const { creditUser, sessions } = require("../models");
const billingServices = require("../service/billingServices");

class billingCycle {
  async store(req, res) {
    try {
      const billingRegister = await billingServices.store({ data: req.data });
  ...
  async index(req, res) {
    ...
    const userPk = await sessions.findByPk(billingDebitId);
    if (!userPk) return res.status(401).json({ error: "nao encontrado" });
    try {
      let getDate = await sessions.findByPk(user_id, {
